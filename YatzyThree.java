...
class YatzyThree {
  public static void main(String[] args) {
    ...
    int keeperVal = doKeeperDiag(playerOneRoll);
    int playerOneKeepVals = keepVal(playerOneRoll+100000,keeperVal);
    System.out.println("P1 keep " + playerOneKeepVals);
  ...
  public static int doKeeperDiag(int diceVals) {
    Scanner myScan = new Scanner(System.in);
    System.out.println("Dit kast var " + diceVals );
    System.out.println("Angiv hvilke du vil beholde:");
    int keepMask = myScan.nextInt();
    return keepMask;
  }
  public static int keepVal(int diceVals, int keepers) {
    ...
    if (keepers%10 == 1) {
      d1=diceVals%10;
      retVal = retVal + d1*10000;
    }
    diceVals = diceVals / 10;
    keepers = keepers / 10;
    if (keepers%10 == 1) {
      d2=diceVals%10;
      retVal = retVal + d2*1000;
    }
    diceVals = diceVals / 10;
    keepers = keepers / 10;
    if (keepers%10 == 1) {
      d3=diceVals%10;
      retVal = retVal + d3*100;
    }
    diceVals = diceVals / 10;
    keepers = keepers / 10;
    if (keepers%10 == 1) {
